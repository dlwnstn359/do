...
st.header("해양 생태계")
st.header("해조류")
st.write("독도 주변 해역은 동해안으로 북상하는 동한난류와 북쪽에서 남하하는 북한한류의 영향을 동시에 받아 해조류 서식에 적합한 환경을 갖는다. 현재까지 독도에서 관찰된 해조류는 녹조류 7과 9속 26종, 갈조류 14과 37속 67종, 홍조류 30과 82속 160종이며 해산종자식물 1종을 포함하여 총 250여종에 달한다.")
st.image("s01020313.jpg")
st.header("해양무척추동물")
st.write("독도의 조간대에는 검은큰따개비, 거북손, 조무래기따개비, 바위게, 갯강구 등의 갑각류와 홍합, 군부, 팽이고둥 등의 연체동물, 자포동물 중 해변말미잘류의 종들이 우점하며, 조하대에는 홍합, 검은테군소, 군소, 큰빨강따개비, 삼각따개비, 태생굴, 블산호류, 무쓰뿌리돌산호, 넓적끈적해면, 불똥해면, 짧은가시거미불가사리 등이 점한다. 서도의 서측 수심 25m 부근에서 나무돌산호과에 속하는 유착나무돌산호의 국내 최대군집 (폭 5m, 높이 3.3m)이 발견되었다. 유착나무돌산호는 남해안의 청정해역에서 흔히 발견 되던 종이였으나, 최근 지속적으로 서식지가 감소")
